...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const ok = login(id.trim(), password.trim(), activeTab);
    if (!ok) setError('ID atau password salah. Coba lagi.');
    setIsLoading(false);
  };
        ...
        <div className="w-full max-w-sm animate-slide-up">
          ...
          <div className="flex gap-1 bg-muted rounded-xl p-1 mb-6">
            {(Object.keys(tabConfig) as Tab[]).map(tab => (
              <button
                key={tab}
                onClick={() => { setActiveTab(tab); setId(''); setPassword(''); setError(''); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{tabConfig[activeTab].passwordHint}</p>
            </div>
